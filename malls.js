...
export function assignKuroaka(roles, maxRed = 3) {
  let redCount = 0;
...
export function tasteGuidance(key, context) {
  if (key === "kuroaka") {
...
// ターゲット年齢層の指示
export function targetGuidance(key) {
  const t = TARGETS[key];
  if (!t) return "";
...
// 商品ジャンル別テイストの指示
export function genreGuidance(key) {
  const g = GENRES[key];
  if (!g) return "";
...
export function subjectPrefix(genreKey) {
...
export function extrasGuidance({ genre, taste, target, notes, context, tasteText } = {}) {
...
export function mallGuidance(key) {
  const m = MALLS[key];
  if (!m) return "";
  return [
    `【出店モール最適化: ${m.label}】`,
...
export function buildProductPagePrompt(key, slot, features, index, total, extras) {
  const m = MALLS[key];
  const lines = [
